...
Storage backend: **Azure Table Storage**, accessed via `Azure.Data.Tables` + `Azure.EntityServices.Tables`. Unit tests use [`Spotflow.InMemory.Azure.Storage`](https://github.com/spotflow-io/in-memory-azure-test-sdk) (in-process fakes for Table Storage + Blob Storage) and Lucene `RAMDirectory` — no external processes needed. Integration tests and local development can use [Azurite](https://github.com/Azure/Azurite).
...
flowchart LR
    ...
    subgraph "Azure Table Storage"
        ...
        ETC --> Tables[(Tables<br/>In-memory / Azurite / Azure)]
    end
...
**Testing:** Unit tests use [`Spotflow.InMemory.Azure.Storage`](https://github.com/spotflow-io/in-memory-azure-test-sdk) for in-process Table Storage fakes and Lucene `RAMDirectory` for in-process search indexes. No external processes, no Docker, no Azurite — just run the tests.

```csharp
// Production
services.AddLottaDB(opts =>
{
    opts.UseAzureTables(productionConnectionString);
    opts.UseLuceneDirectory(new FSDirectoryProvider("./lucene-indexes"));
});

// Unit tests — fully in-process
services.AddLottaDB(opts =>
{
    opts.UseInMemoryTables();    // Spotflow in-memory fakes
...
/test
  LottaDB.Tests                    // in-process (Spotflow + RAMDirectory)
```
...
| Azure Table Storage (in-memory fakes for tests, Azurite for integration) | One backend, one code path. Unit tests fully in-process via Spotflow + RAMDirectory. |
