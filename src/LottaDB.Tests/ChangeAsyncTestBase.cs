...
public abstract class ChangeAsyncTestBase : LottaTestBase
{
    ...
    public async Task ChangeAsync_ParallelWriters_NoLostUpdates()
    {
        ...
        Assert.True(N == final!.Counter, $"GetAsync() Counter is wrong for {base.Catalog.GetTableClient().Name}: expected {N}, got {final.Counter}");
        var final2 = db.Search<Actor>(a => a.Username == final.Username).Single();
        Assert.True(N == final2!.Counter, $"Search() Counter is wrong for {base.Catalog.GetTableClient().Name}: expected {N}, got {final2.Counter}");
    }
